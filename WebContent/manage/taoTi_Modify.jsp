<%@ page language="java" contentType="text/html; charset=utf-8" import="java.util.*"
 ...
 <%
     ...
     TaoTi tao = tsi.getTao(id);
     String Name=null;
     if((Name=request.getParameter("name"))!=null){
    	 int LessonId = Integer.parseInt(request.getParameter("lessonId"));
    	 if(tsi.edit(id, Name, LessonId)==1){
    		 %>
...
function fun(){
	  document.getElementById("lessonId").value="<%=tao.getLessonId()%>";
}
</script>
<body >
...
<form action="taoTi_Modify.jsp" method="post" onsubmit="return check()">
  ...
  <tr align="center">
    ...
    <td width="73%" align="left">
      <input type="text" name="name" size="30" value="<%=tao.getName() %>" />
	  <input type="hidden" name="ID">
	  </td>
    <tr>
    <td height="30" align="left" style="padding:5px;">所属课程：</td>
    <td align="left">
    <input type="hidden" name="id" value="<%=tao.getId()%>">
    <select name="lessonId" id="lessonId">
    	<%for(int i=0;i<list.size();i++) {%>
	 		<option value="<%=list.get(i).getId()%>"
	 		<%if(list.get(i).getId()==tao.getLessonId()) {%>
	 			selected
	 		<%} %>
